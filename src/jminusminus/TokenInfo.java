...
enum TokenKind {
    EOF("<EOF>"), ABSTRACT("abstract"), BOOLEAN("boolean"), CHAR("char"), CLASS("class"), ELSE("else"), EXTENDS("extends"), FALSE("false"),
    		IF("if"), IMPORT("import"), INSTANCEOF("instanceof"), INT("int"), NEW("new"), NULL("null"), PACKAGE("package"), PRIVATE("private"),
    		PROTECTED("protected"), PUBLIC("public"), RETURN("return"), STATIC("static"), SUPER("super"), THIS("this"), TRUE("true"),
    		VOID("void"), WHILE("while"), CONTINUE("continue"), FOR("for"), SWITCH("switch"), ASSERT("assert"), DEFAULT("default"), 
    		GOTO("goto"), SYNCHRONIZED("synchronized"), DO("do"), BREAK("break"), DOUBLE("double"), IMPLEMENTS("implements"), THROW("throw"),
    		BYTE("byte"), THROWS("throws"), CASE("case"), ENUM("enum"), TRANSIENT("transient"), CATCH("catch"), SHORT("short"), TRY("try"), 
    		FINAL("final"), INTERFACE("interface"), FINALLY("finally"), LONG("long"), STRICTFP("strictfp"), VOLATILE("volatile"), CONST("const"), 
    		FLOAT("float"), NATIVE("native"), PLUS("+"), AND("&"), AND_ASSIGN("&="), ASSIGN("="), COMPLIMENT("~"), DEC("--"), DIV("/"), 
    		DIV_ASSIGN("/="), EQUAL("=="), GE(">="), GT(">"), INC("++"), LAND("&&"), LE("<="), LT("<"), LNOT("!"), LOR("||"), LSHIFT("<<"), 
    		LSHIFT_ASSIGN("<<="), MINUS("-"), MINUS_ASSIGN("-="), MOD("%"), MOD_ASSIGN("%="), NEQUAL("!="), OR("|"), OR_ASSIGN("|="), 
    		PLUS_ASSIGN("+="), RSHIFT(">>"), RSHIFT_ASSIGN(">>="), RSHIFTFILL(">>>"), RSHIFTFILL_ASSIGN(">>>="), STAR("*"), STAR_ASSIGN("*="), 
    		XOR("^"), XOR_ASSIGN("^="), CONDITIONAL("?"), LPAREN("("), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["), RBRACK(
            "]"), SEMI(";"), COMMA(","), DOT("."), COLON(":"), IDENTIFIER("<IDENTIFIER>"), INT_LITERAL("<INT_LITERAL>"), CHAR_LITERAL("<CHAR_LITERAL>"),
    		STRING_LITERAL("<STRING_LITERAL>"), DOUBLE_LITERAL("<DOUBLE_LITERAL>"), FLOAT_LITERAL("<FLOAT_LITERAL>"), LONG_LITERAL("<LONG_LITERAL>"),
