...
/**
 * The AST node for an non equality (!=) expression. Implements short-circuiting
 * branching.
 */
...
class JNotEqualOp extends JBooleanBinaryExpression {
    ...
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        ...
        rhs.codegen(output);
        if (lhs.type().isReference()) {
            output.addBranchInstruction(onTrue ? IF_ICMPEQ : IF_ACMPNE,
                    targetLabel);
        } else {
            output.addBranchInstruction(onTrue ? IF_ACMPNE : IF_ICMPEQ,
                    targetLabel);
        }
    }
}
