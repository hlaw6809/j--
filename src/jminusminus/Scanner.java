    ...
    public TokenInfo getNextToken() {
                ...
                if (ch == '/') {
                	...
                	while (multiLineComment) {
                		...
                		if (ch == '*') {
                			...
                			reportScannerError("Multi-Line comment never ended.");
                		}
                	}
                } else {
                	if (ch == '=') {
        ...
        case '>':
            ...
            if (ch == '=') {
            	...
            	if (ch == '>') {
            		...
            		if (ch == '=') {
            			nextCh();
            			return new TokenInfo(RSHIFTFILL_ASSIGN, line);
            		} else {
            			return new TokenInfo(RSHIFTFILL, line);
            		}
            	} else if (ch == '=') {
            		nextCh();
        ...
        case '9':
            ...
            if (ch == 'L') {
            	...
            	nextCh();
            	if (!isDigit(ch)) {
            		reportScannerError("Unexpected end of decimal.");
            	}
            	while(isDigit(ch)) {
            		buffer.append(ch);
            		nextCh();
        ...
        default:
            if (isIdentifierStart(ch)) {
                ...
                if (reserved.containsKey(identifier)) {
                	TokenKind reservedKind = reserved.get(identifier);
                	if (reservedKind == TRUE || reservedKind == FALSE) {
                		return new TokenInfo(BOOLEAN_LITERAL, reservedKind.image(), line);
                	} else {
                		return new TokenInfo(reservedKind, line);
                	}
                } else {
